...
| `watermark_invisible.py` | Layer ① — `embed`, plus **two read paths with different authority**: the CRC envelope (integrity-checked, blind discovery) and the checksum-less raw path (a *clue* only). The carrier is `config.CARRIER_CHANNEL`/`CARRIER_SCALE` (luma at step 72 since P10) and the read side also tries `config.LEGACY_CARRIERS`; `_CarrierEmbed` additionally fixes the library's H/V detail-band swap (P11). The block scan transcribes `imwatermark.dwtDctSvd` and is proven byte-exact per carrier (spike P3-B) — a carrier mismatch scrambles the reading, so never hardcode one. | Yes (whole-file) |
