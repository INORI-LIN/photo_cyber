...
- **边际配置下比特判决可能不可逐次复现**（落地时发现）：某格「(0,72) + 256×256 + 14 字节」在一次测量中失败、随后 3 次重复与同进程连续重跑均通过。怀疑是 `cv2.dct`/`np.linalg.svd` 的多线程归约顺序在**判决边界上**的 1-ulp 差异翻转了个别比特。若成立，则「66/66」在每比特块数很少的长度上不是逐次可复现的，且 P6 的基准与 P4 的自检都会受此影响。需专门验证：固定图像与 payload，单线程重复 N 次，统计翻转次数与发生在哪些长度。**在此之前不要把 66/66 当作硬保证引用。**
